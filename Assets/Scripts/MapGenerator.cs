...
public class MapGenerator : MonoBehaviour
{
    ...
    public const int mapChunkSize = 239;        // 255 max by unity, mesh dimensions are one fewer (-1), 240 has nice factors (adding 2)

    [Range(0, 6)]                               // 1 - no simplification, 2,4,5,6,10, or 12 for more and more simplification
    public int previewLevelOfDetail;            // double for our "increment" value (mapping 0 to 1)
    ...
    public void RequestMeshData(MapData mapData, int levelOfDetail, Action<MeshData> callback)
    {
        ...
        {
            MeshData meshData = MeshGenerator.GenerateTerrainMesh(mapData.heightMap, meshHeightMultiplier, meshHeightCurve, previewLevelOfDetail);
            callback(meshData);
        }
    }
    ...
    private void OnValidate()
    {
        ...
        // since Awake is not called if the game is not started, while in the editor ...
        falloffMap = FalloffGenerator.GenerateFalloffMap(mapChunkSize);
    }

    struct MapThreadInfo<T> // for MapData and MeshData
    {

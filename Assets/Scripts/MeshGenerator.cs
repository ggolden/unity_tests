...
{
    public static MeshData GenerateTerrainMesh(float[,] heightMap, float heightMultiplier, AnimationCurve meshHeightCurveMaster, int levelOfDetail)
    {
        // AnimationCurve is not thread safe, so copy it so the thread has its own
        AnimationCurve meshHeightCurve = new AnimationCurve(meshHeightCurveMaster.keys);
...
public class MeshData
{
    ...
    public MeshData(int verticesPerLine)
    {
        ...
        triangles = new int[(verticesPerLine - 1) * (verticesPerLine - 1) * 2 * 3];

        borderVerticies = new Vector3[verticesPerLine * 4 + 4];
